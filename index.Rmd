...
summarize_all(diags, mean)
```
The non - parametric classification that was used was the K - nearest neighbor technique in order to predict if the movie reviewed was of the action genre or not based on the length, budget, rating, and votes for the movie. This classification yielded even more poor results than the linear classification done prior as the AUC value fell to 0.5112, which is an significant decrease from the AUC value of 0.72068, as it is now not a passing value. This means the in- sample assessment without the influence of set conditions such as mean, standard deviation, variance, etc. of this classification yielded a worse prediction of the outcome of the binary variable, Action, based on length, budget, rating, and votes. This poor outcome is adequately demonstrated in the ROC plot as the curve aligns with the the 45 degree angle of the plane much more than the previous ROC plot, indicating very low accuracy of this classification. To further validate this notion, a cross validation was performed, and the AUC value increased dramatically to .87005, which overall concludes that this non-parametric model did a poor job at predicting the effects of the action movie genre predictor. However, because of this increase of the AUC value in the cross-validation, this could be a sign of overfitting. 
