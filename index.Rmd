...
for(i in 2:10){  
  kms <- kmeans(num_dat,centers=i) #compute k-means solution for each k
  sil <- silhouette(kms$cluster,dist(num_dat)) #get sil widths
...
fit <- lm(rating ~ length + budget, data = randomized)  #predict mpg from all other variables
...
data <- randomized[sample(nrow(randomized)), ]  #randomly order rows
...
This project allowed me to access a data set I have been wanting to explore since before I began project 1, because I really enjoy how much can be done with movie rating data, as an opinionated movie watcher like myself.While the coding delve in to concepts that, at first I struggled with, I was able to enjoy it and understand it more with this data set, particularly because of its usability with binary variables in relation to the topics in this project. In the future, I would like to further explore the use of R in conjunction with python and play with this interaction between the two and gather more sufisticated findings. 
